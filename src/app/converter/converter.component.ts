...
export class ConverterComponent implements OnInit {
  ...
  convert() {
    ...
    } else if (formValues.from === formValues.to) {
      this.toastr.error('selected from and to input are same' , 'Currencies selected are same');
      this.converted = false;
    } else if (formValues.amount < 1) {
      this.toastr.error('amount should not be less than one' , 'Amount input is wrong');
      this.converted = false;
    } else {
